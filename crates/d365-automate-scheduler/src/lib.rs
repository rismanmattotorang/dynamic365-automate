...
//!
//! Declared Dynamics 365-typical monitoring jobs:
//!   - every Monday morning: summarise the Solution Checker findings created
//!     in the last seven days and post to a Teams channel
//!   - every quarter: rank Dataverse solutions by years-to-EOL and flag
//!     those within four years
//!   - hourly during business hours: scan deployable packages stuck in UAT
//!
...
mod tests {
    ...
    async fn parse_config_and_fire() {
        let toml = r#"
            [[jobs]]
            name = "solution_checker_weekly"
            tool = "d365.docs.search"
            channel = "teams:#fin-ops"
            arguments = { query = "Solution Checker findings last week" }
