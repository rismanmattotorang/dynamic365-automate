...
//!
//! Phase 1A focuses on a single source family (Microsoft Learn HTML pages).  The
//! crawler is split into two surfaces:
//!   1. `parse_learn_html` — pure function that turns one HTML page
//!      into a `ParsedPage`.  Has a dedicated test suite that runs nightly
//!      against a snapshot corpus (paper §X-N risk-1).
//!   2. `LearnCrawler` — orchestrator that walks a directory or an
//!      HTTP root, calls the parser, and yields `Document`s.
//!
//! Real production crawling against learn.microsoft.com requires user agent +
...
use d365_automate_kb::{Document, Domain};
...
pub fn parse_learn_html(raw: &str) -> Result<ParsedPage, CrawlError> {
...
pub struct LearnCrawler {
    /// Domain assigned to all documents produced by this crawler.  Defaults
    /// to `Learn` but is configurable so the same parser can drive X++
    /// HTML dumps or Power Automate flow exports.
    pub domain: Domain,
}

impl LearnCrawler {
    pub fn new() -> Self { Self { domain: Domain::Learn } }
    ...
    pub async fn fetch_url(
        ...
        let page = parse_learn_html(&body)?;
...
<html>
<head>
  <title>Financial period close in Dynamics 365 Finance - Microsoft Learn</title>
  <meta name="module" content="GeneralLedger"/>
</head>
<body>
  <nav class="breadcrumb">
    <a>Finance</a> &gt; <a>General Ledger</a> &gt; <a>Period Close</a>
  </nav>
  <h1>Financial period close in Dynamics 365 Finance</h1>
  <main>
    <p>Open and close ledger periods via FiscalCalendarPeriod.</p>
    <p>Execute foreign-currency revaluation.</p>
    <p>Post accruals and deferrals; run LedgerJournalTrans -> GeneralJournalAccountEntry reconciliation.</p>
  </main>
</body>
</html>
"#;
    ...
    fn parses_title_breadcrumb_body() {
        let p = parse_learn_html(SAMPLE).unwrap();
        assert_eq!(p.title, "Financial period close in Dynamics 365 Finance");
        assert_eq!(p.breadcrumbs, vec!["Finance", "General Ledger", "Period Close"]);
        assert!(p.body.contains("FiscalCalendarPeriod"));
        assert!(p.body.contains("GeneralJournalAccountEntry"));
        assert_eq!(p.module.as_deref(), Some("GeneralLedger"));
    }

    #[test]
    fn rejects_empty_title() {
        let html = "<html><body><p>no title here</p></body></html>";
        let err = parse_learn_html(html).unwrap_err();
        match err {
