...
use d365_automate_kb::{Document, KnowledgeStore, UpsertBatch};
use std::sync::Arc;
use thiserror::Error;
...
pub enum PipelineError {
    ...
    #[error("store: {0}")]
    Store(#[from] d365_automate_kb::StoreError),
}
...
mod tests {
    ...
    use d365_automate_kb::{Domain, InMemoryKb, SearchQuery};
    ...
    async fn end_to_end_ingest_then_search() {
        ...
        let docs = vec![
            {
                let mut d = Document::new(
                    "learn:finance/period-close", Domain::Learn,
                    "d365-learn://finance/period-close",
                    "Financial period close in Dynamics 365 Finance",
                    "Open and close ledger periods via the FiscalCalendarPeriod entity. Execute foreign-currency revaluation. Post accruals and deferrals; reconcile LedgerJournalTrans to GeneralJournalAccountEntry.",
                );
                d.breadcrumbs = vec!["Finance".into(), "General Ledger".into()];
                d
            },
            Document::new(
                "learn:scm/product-receipt", Domain::Learn,
                "d365-learn://scm/product-receipt",
                "Product receipt posting",
                "Post product receipts against a purchase order. Journal types: arrival receipt, reversal, and return.",
            ),
        ];
        ...
        let q_vec = embedder
            .embed(&["period close GeneralJournalAccountEntry reconciliation".to_string()])
            .await
            .unwrap();
        let hits = store
            .search(
