...
use d365_automate_kb::{Chunk, Document};
...
pub struct ChunkerConfig {
    ...
    /// Microsoft Learn corpus in §VI-E.
...
/// call per document.
fn document_context(doc: &d365_automate_kb::Document) -> String {
    let body = doc.body.trim();
...
mod tests {
    ...
    fn breadcrumb_is_prepended() {
        let mut doc = Document::new(
            ...
            "Period-End Close",
            "Open ledger periods via FiscalCalendarPeriod. Execute foreign-currency revaluation.",
        );
    ...
    fn long_body_splits_into_multiple_chunks() {
        ...
        let doc = Document::new("d:1", Domain::Learn, "u", "T", body);
        let chunks = chunk_document(&doc, &ChunkerConfig::default());
