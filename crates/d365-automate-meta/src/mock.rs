...
impl MockMetadataClient {
    ...
    fn seed(&mut self) {
        ...
        // Models
        self.models.insert("GTFin".into(), ModelContents {
            model: "GTFin".into(),
            description: Some("GaussianTech Finance customisations".into()),
            members: vec![
                ModelMember { name: "GTFinPostJournal".into(), kind: XppObjectKind::Job, description: Some("Post GL journals".into()) },
                ModelMember { name: "GTFinJournalPoster".into(), kind: XppObjectKind::Class, description: Some("GL posting helper".into()) },
                ModelMember { name: "GTFinPostable".into(), kind: XppObjectKind::Interface, description: Some("GL posting contract".into()) },
                ModelMember { name: "GTFinConstants".into(), kind: XppObjectKind::Class, description: Some("Shared constants".into()) },
                ModelMember { name: "GTFinValidation".into(), kind: XppObjectKind::Class, description: Some("Posting validation".into()) },
            ],
        });
        self.models.insert("GTScm".into(), ModelContents {
            model: "GTScm".into(),
            description: Some("GaussianTech Supply Chain customisations".into()),
            members: vec![
                ModelMember { name: "GTScmGrnCheck".into(), kind: XppObjectKind::Job, description: Some("Product receipt reconciliation".into()) },
            ],
        });

        // Cross-reference (who uses what)
        self.cross_refs.insert(("GTFinPostable".into(), XppObjectKind::Interface), vec![
            CrossReferenceHit { object: "GTFinJournalPoster".into(), kind: XppObjectKind::Class, location: "class declaration".into(), usage: "implements".into() },
        ]);
        self.cross_refs.insert(("GTFinJournalPoster".into(), XppObjectKind::Class), vec![
            CrossReferenceHit { object: "GTFinPostJournal".into(), kind: XppObjectKind::Job, location: "main()".into(), usage: "call".into() },
        ]);
        self.cross_refs.insert(("LedgerJournalTrans".into(), XppObjectKind::Table), vec![
            CrossReferenceHit { object: "LedgerJournalLineEntity".into(), kind: XppObjectKind::DataEntity, location: "data source".into(), usage: "read".into() },
            CrossReferenceHit { object: "GTFinJournalPoster".into(), kind: XppObjectKind::Class, location: "post()".into(), usage: "write".into() },
        ]);

        // Entity data (for get_entity_contents)
        self.entity_data.insert("LedgerJournalLineEntity".into(), vec![
            EntityRow { values: serde_json::json!({
...
impl MetadataClient for MockMetadataClient {
    fn connection(&self) -> &D365Connection { &self.connection }

    async fn get_class(&self, name: &str) -> MetaResult<ObjectSource> {
        self.classes.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Class".into(), name: name.into() })
    }
    async fn get_interface(&self, name: &str) -> MetaResult<ObjectSource> {
        self.interfaces.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Interface".into(), name: name.into() })
    }
    async fn get_table(&self, name: &str) -> MetaResult<ObjectSource> {
        self.tables.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Table".into(), name: name.into() })
    }
    async fn get_job(&self, name: &str) -> MetaResult<ObjectSource> {
        self.jobs.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Job".into(), name: name.into() })
    }
    async fn get_form(&self, name: &str) -> MetaResult<ObjectSource> {
        self.forms.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Form".into(), name: name.into() })
    }
    async fn get_model_contents(&self, model: &str) -> MetaResult<ModelContents> {
        self.models.get(model).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "Model".into(), name: model.into() })
    }
    async fn get_data_entity(&self, name: &str) -> MetaResult<DataEntityView> {
        self.data_entities.get(name).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "DataEntity".into(), name: name.into() })
    }

    async fn search(&self, request: MetaSearchRequest) -> MetaResult<Vec<MetaSearchHit>> {
        let q = request.query.to_lowercase();
        let terms: Vec<&str> = q.split_whitespace().collect();
        let mut hits: Vec<MetaSearchHit> = self.search_all()
            .filter(|(o,)| request.kind.is_none_or(|k| k == o.kind))
            .filter_map(|(o,)| {
                let hay = format!("{} {}", o.name.to_lowercase(), o.description.as_deref().unwrap_or("").to_lowercase());
                let score: usize = terms.iter().map(|t| hay.matches(t).count()).sum();
                if score == 0 { None } else {
                    Some(MetaSearchHit {
                        name: o.name.clone(), kind: o.kind, description: o.description.clone(),
                        model: o.model.clone(), score: score as f32,
                    })
                }
            })
            .collect();
    ...
    async fn cross_reference(&self, request: CrossReferenceRequest) -> MetaResult<Vec<CrossReferenceHit>> {
        Ok(self.cross_refs.get(&(request.name.clone(), request.kind)).cloned().unwrap_or_default())
    }
    ...
    async fn get_entity_contents(&self, entity: &str, max_rows: usize) -> MetaResult<Vec<EntityRow>> {
        ...
        let mut rows = self.entity_data.get(entity).cloned()
            .ok_or_else(|| MetaError::NotFound { kind: "DataEntity".into(), name: entity.into() })?;
    ...
    async fn deploy(&self, request: DeployRequest, ctx: MetaCallContext) -> MetaResult<DeployOutcome> {
        ...
        if !known {
            return Err(MetaError::NotFound { kind: request.kind.label().into(), name: request.name });
        }
        Ok(DeployOutcome {
            name: request.name.clone(),
            kind: request.kind,
            deployed: true,
            messages: vec![format!("{} '{}' built and synchronised (mock)", request.kind.label(), request.name)],
        })
    }
}
...
mod tests {
    ...
    async fn search_finds_posting_objects() {
        let c = client();
        let hits = c.search(MetaSearchRequest { query: "post".into(), kind: None, max_results: 10 }).await.unwrap();
    ...
    async fn cross_reference_finds_implementers() {
        let c = client();
        let refs = c.cross_reference(CrossReferenceRequest {
            name: "GTFinPostable".into(), kind: XppObjectKind::Interface,
        }).await.unwrap();
        assert!(refs.iter().any(|r| r.object == "GTFinJournalPoster" && r.usage == "implements"));
    }
    ...
    async fn deploy_is_blocked_in_read_only_mode() {
        let c = client();
        let err = c.deploy(
            DeployRequest { name: "GTFinJournalPoster".into(), kind: XppObjectKind::Class },
    ...
    async fn entity_contents_over_cap_is_blocked() {
        let c = client();
        let err = c.get_entity_contents("LedgerJournalLineEntity", 5000).await.unwrap_err();
        assert!(matches!(err, MetaError::EntityDataBlocked(_)), "got {err:?}");
    }
}
