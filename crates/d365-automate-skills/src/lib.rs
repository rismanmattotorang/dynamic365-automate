...
//! and prompt engineering for a specific Dynamics 365 scenario.  Agents invoke
//! skills via MCP `prompts/get`.  Each skill ships as a markdown file with YAML
//! frontmatter:
//!
//! ```text
//! ---
//! name: d365.skill.period-close-investigation
//! description: Investigate root causes of a ledger period-close delay
//! tags: [finance, period-close, investigation]
//! requires_tools: [d365.docs.search, d365.entity.read, xpp.meta.cross_reference]
//! arguments:
//!   - name: legal_entity
//!     description: DataAreaId, e.g. "USMF"
//!     required: true
//!   - name: fiscal_period
...
//! ---
//!
//! Investigate the ledger period close for {{legal_entity}} ({{fiscal_period}}).
//!
//! Steps:
//! 1. Use d365.docs.search to find the official period-close procedure ...
//! ```
//!
...
mod tests {
    ...
    async fn parses_frontmatter_and_body() {
        ...
        let content = r#"---
name: d365.skill.test
description: A test skill
tags: [test, demo]
requires_tools: [d365.docs.search]
arguments:
  - name: legal_entity
...
Investigate period close for {{legal_entity}}.
"#;
        tokio::fs::write(&path, content).await.unwrap();
        let skill = parse_skill_file(&path).await.unwrap();
        assert_eq!(skill.name, "d365.skill.test");
        assert_eq!(skill.tags, vec!["test", "demo"]);
        assert_eq!(skill.requires_tools, vec!["d365.docs.search"]);
    ...
    fn render_substitutes_arguments() {
        let s = Skill {
            ...
            arguments: vec![
                SkillArgument { name: "legal_entity".into(), description: None, required: true },
                SkillArgument { name: "optional".into(), description: None, required: false },
            ],
            body: "Code: {{legal_entity}}, Opt: {{optional}}".into(),
            source: None,
        };
        let mut args = HashMap::new();
        args.insert("legal_entity".into(), "1000".into());
    ...
    fn render_flags_missing_required() {
        let s = Skill {
            ...
            body: "Code: {{legal_entity}}".into(),
            source: None,
        };
        let out = s.render(&HashMap::new());
        assert!(out.contains("<MISSING legal_entity>"));
    }
}
