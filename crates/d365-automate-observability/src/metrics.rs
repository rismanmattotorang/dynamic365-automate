...
//!
//! Designed for the D365-Automate hot path: every tool call records a
...
/// Standard D365-Automate histogram buckets in seconds.  Chosen to
...
mod tests {
    ...
    fn counter_increments_and_renders() {
        ...
        r.register("d365_service_calls_total", MetricKind::Counter, "Total OData service calls");
        r.inc_counter("d365_service_calls_total", &[("operation", "ReleasedProductGetDetail")]);
        r.inc_counter("d365_service_calls_total", &[("operation", "ReleasedProductGetDetail")]);
        r.inc_counter("d365_service_calls_total", &[("operation", "EntityRead")]);
        let out = r.render();
        assert!(out.contains("# TYPE d365_service_calls_total counter"));
        assert!(out.contains("d365_service_calls_total{operation=\"ReleasedProductGetDetail\"} 2"));
        assert!(out.contains("d365_service_calls_total{operation=\"EntityRead\"} 1"));
    }
    ...
    fn histogram_records_buckets_correctly() {
        ...
        r.observe_histogram("mcp_tool_latency_seconds", &[("tool", "d365.docs.search")], 0.0003);
        r.observe_histogram("mcp_tool_latency_seconds", &[("tool", "d365.docs.search")], 0.07);
        r.observe_histogram("mcp_tool_latency_seconds", &[("tool", "d365.docs.search")], 0.2);
        let out = r.render();
    ...
    fn gauge_replaces_value() {
        let r = MetricsRegistry::new();
        r.register("d365_pool_in_use", MetricKind::Gauge, "Pool slots in use");
        r.set_gauge("d365_pool_in_use", &[], 5.0);
        r.set_gauge("d365_pool_in_use", &[], 3.0);
        let out = r.render();
        assert!(out.contains("d365_pool_in_use 3"));
        assert!(!out.contains("d365_pool_in_use 5"));
    }
