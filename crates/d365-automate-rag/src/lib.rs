...
// can include it in `d365.docs.search` responses.

use d365_automate_kb::{Domain, KnowledgeStore, SearchHit, SearchQuery};
...
/// it picked — invaluable for Dynamics 365 queries where exact entity names,
/// service names, and field identifiers matter as much as semantic similarity.
///
...
pub struct RetrievalDiagnostics {
    ...
    /// after the identifier-preserving tokenizer.  Surfacing this
...
impl RagEngine {
    ...
    pub async fn search<'a>(&self, query: Query<'a>) -> Result<Vec<Hit>, d365_automate_kb::StoreError> {
        let resp = self.hybrid_search(query).await?;
        let hits = resp.hits.into_iter().map(|view| Hit {
            hit: SearchHit {
                chunk: d365_automate_kb::Chunk {
                    id: view.document_id.clone(),
                    document_id: view.document_id,
                    domain: Domain::Learn, // placeholder; reconstruct from store on demand
                    ordinal: 0,
                    text: view.snippet,
                    embedding: None,
                    breadcrumbs: Vec::new(),
    ...
    pub async fn hybrid_search<'a>(&self, query: Query<'a>) -> Result<SearchResponse, d365_automate_kb::StoreError> {
...
mod tests {
    ...
    #[test]
    fn tokenize_query_preserves_d365_identifiers() {
        let t = tokenize_query("LedgerGeneralJournalEntry and the period close");
        // Entity identifier survives intact as a single token.
        assert!(t.iter().any(|x| x == "ledgergeneraljournalentry"));
