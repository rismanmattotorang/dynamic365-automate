...
//!
//! Retry classification is baked into the trait surface: [`D365Error::is_transient`]
//! decides whether [`retry_with_backoff`] retries. Agents never waste tokens
//! reasoning about transient environment unavailability.
...
pub async fn retry_with_backoff<F, Fut, T>(policy: &BackoffPolicy, mut f: F) -> D365Result<T>
where
    F: FnMut() -> Fut,
    Fut: Future<Output = D365Result<T>>,
{
...
impl CircuitBreaker {
    ...
    /// transition to half-open so the next call can probe.
    ...
    pub async fn call<F, Fut, T>(&self, f: F) -> D365Result<T>
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = D365Result<T>>,
    {
...
mod tests {
    ...
    async fn retries_transient_errors() {
        ...
        let result: D365Result<&'static str> = retry_with_backoff(&policy, move || {
            let a = Arc::clone(&a);
            async move {
    ...
    async fn does_not_retry_permanent_errors() {
        let attempts = Arc::new(AtomicU32::new(0));
        let a = Arc::clone(&attempts);
        let result: D365Result<()> = retry_with_backoff(&BackoffPolicy::default(), move || {
            let a = Arc::clone(&a);
            async move {
                a.fetch_add(1, Ordering::SeqCst);
                Err(D365Error::NotFound("ReleasedProductsV9".into()))
            }
        })
        .await;
    ...
    async fn breaker_opens_after_threshold() {
        ...
        assert!(matches!(r, Err(D365Error::CircuitOpen { .. })));
    }
}
