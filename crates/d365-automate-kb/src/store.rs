...
impl InMemoryKb {
    ...
    /// Paper §VII-C / VII-D: tuned for the Dynamics 365 corpus where exact-match
    /// retrieval of entity names and X++ identifiers matters as much
...
/// alphanumeric-or-underscore runs; preserves Dynamics 365 identifier shape
/// (`LedgerGeneralJournalEntry`, `ReleasedProductsV2`).
...
mod tests {
    ...
    async fn text_search_filters_by_domain() {
        ...
        kb.insert_chunk(sample_chunk("a", Domain::Learn, "period close in FI", None));
        kb.insert_chunk(sample_chunk("b", Domain::Xpp, "period close routine", None));

        let hits = kb.search(SearchQuery::text("period close", 10).with_domain(Domain::Learn))
            .await
            .unwrap();
    ...
    async fn vector_search_orders_by_cosine() {
        let kb = InMemoryKb::new();
        kb.insert_chunk(sample_chunk("near", Domain::Learn, "x", Some(vec![1.0, 0.0, 0.0])));
        kb.insert_chunk(sample_chunk("far",  Domain::Learn, "y", Some(vec![0.0, 1.0, 0.0])));
    ...
    async fn upsert_dedup_skips_identical_chunk() {
        let kb = InMemoryKb::new();
        let chunk = sample_chunk("c-1", Domain::Learn, "identical body", None);
    ...
    async fn upsert_dedup_treats_changed_text_as_new() {
        let kb = InMemoryKb::new();
        let c1 = sample_chunk("c-1", Domain::Learn, "v1", None);
        let c2 = sample_chunk("c-1", Domain::Learn, "v2", None);
        kb.insert_chunk(c1);
        kb.insert_chunk(c2);
