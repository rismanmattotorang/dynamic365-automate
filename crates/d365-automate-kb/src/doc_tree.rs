...
//!
//! For D365-Automate we adopt the *data structure* — a typed tree with
...
//! tree at query time via the `d365.kb.navigate` MCP tool that consumes it.
//!
...
//!     extracted Microsoft Learn pages.
//!
...
/// Plain-text section markers used in extracted Microsoft Learn pages and X++
...
mod tests {
    ...
    fn mk(body: &str) -> Document {
        Document::new("d:1", Domain::Learn, "u://1", "Title", body)
    }
...
";
    ...
    fn numbered_section_markers_build_a_tree() {
        let body = "\
1. Period close
Open ledger periods via FiscalCalendarPeriod.
1.1. Foreign currency
Run FX revaluation.
1.2. Reconciliation
LedgerJournalTrans to GeneralJournalAccountEntry reconciliation.
";
        let tree = build_document_tree(&mk(body));
