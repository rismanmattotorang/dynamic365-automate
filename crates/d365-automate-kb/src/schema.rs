...
//!
//! Every Microsoft Learn page, X++ object, Power Automate flow, and Dataverse
//! solution is a first-class `Document` with stable URI + rich metadata.
//! Documents are split into one or more `Chunk`s for embedding and retrieval;
//! each chunk preserves its parent document linkage and breadcrumb so reranking
...
/// `learn:finance/period-close` or `xpp:GTFin/GTFinPostJournal`.
