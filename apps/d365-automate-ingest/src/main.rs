...
//! Microsoft Learn snapshot should leave the store able to answer intent
//! queries like "period close" by returning the right page.

use clap::{Parser, ValueEnum};
use d365_automate_ingest::{
    EmbeddingClient, IngestionPipeline, LearnCrawler, MockEmbedder, OpenAiEmbedder,
};
use d365_automate_kb::{InMemoryKb, KnowledgeStore, SearchQuery};
use std::sync::Arc;
...
#[derive(Parser)]
#[command(
    name = "d365-automate-ingest",
    about = "Crawl, chunk, embed, and upsert Dynamics 365 knowledge into the configured store."
)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    let store: Arc<dyn KnowledgeStore> = match cli.backend {
        ...
        Backend::Qdrant => {
            let q = d365_automate_kb::QdrantStore::new(cli.qdrant_url.clone(), cli.embedding_dim);
