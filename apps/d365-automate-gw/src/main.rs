...
//!
//! Acceptance gate: a Teams-initiated `d365.docs.search`
//! query returns a cited answer within the existing P95 budget plus a
//! 50 ms gateway tax.
//!
//! This binary:
//!   1. Spawns the D365-Automate MCP server as a child process over stdio.
//!   2. Registers channel adapters (CLI now; Teams / Slack / Telegram
//!      land via the trait in `d365-automate-channels`).
...
use d365_automate_channels::{
    ChannelKind, ChannelRegistry, CliChannel, IncomingMessage, OutgoingMessage, now_ms,
};
use d365_automate_memory::{MemoryEntry, MemoryManager, Tier};
use d365_automate_scheduler::{JobExecutor, Scheduler, ScheduledJob};
use std::sync::Arc;
...
#[derive(Parser)]
#[command(
    name = "d365-automate-gw",
    about = "Multi-channel gateway routing channel events into MCP tool calls."
)]
struct Cli {
    /// Path to the D365-Automate MCP server binary.
    #[arg(long, default_value = "target/release/d365-automate-server")]
    server: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    let init = mcp.initialize(
        Implementation { name: "d365-automate-gw".into(), version: env!("CARGO_PKG_VERSION").into() },
...
async fn route(
    ...
    //
    // Convergent insight: agents should
    ...
    let (tool, args) = if lc.contains("solution checker") || lc.contains("best practice") {
        ("d365.docs.search", serde_json::json!({
            "query": "Solution Checker best-practice findings recent",
            "top_k": 3
        }))
    } else if lc.contains("impact") || lc.contains("cross reference") || lc.contains("cross-reference") || lc.contains("depends") {
...
fn match_skill(lc: &str) -> Option<(&'static str, Option<serde_json::Value>)> {
    if (lc.contains("sod") || lc.contains("segregation of duties") || lc.contains("security audit"))
        && (lc.contains("audit") || lc.contains("review") || lc.contains("sod"))
    {
        ...
        return Some(("d365.skill.security_sod_audit", Some(serde_json::json!({
            "user_or_role": target,
            "scope": "user"
        }))));
    }
    if lc.contains("period close") || lc.contains("period-close") || (lc.contains("closing") && lc.contains("ledger")) {
        return Some(("d365.skill.period_close_investigation", Some(serde_json::json!({
            "legal_entity": "USMF"
        }))));
    }
    if lc.contains("code review") || (lc.contains("review") && (lc.contains("x++") || lc.contains("xpp") || lc.contains("class") || lc.contains("job"))) {
        return Some(("d365.skill.xpp_code_review", Some(serde_json::json!({
            "object_name": "GTFinJournalPoster",
            "kind": "class"
        }))));
    }
    if lc.contains("data entity") || (lc.contains("odata") && (lc.contains("design") || lc.contains("expose"))) {
        return Some(("d365.skill.data_entity_design", Some(serde_json::json!({
            "entity_name": "PurchaseOrderHeadersV2"
        }))));
    }
    if lc.contains("deploy") && (lc.contains("impact") || lc.contains("package") || lc.contains("analyse") || lc.contains("analyze")) {
        return Some(("d365.skill.deploy_impact_analysis", Some(serde_json::json!({
            "package": "GTFin-2026.04"
        }))));
    }
    if lc.contains("over-layering") || lc.contains("over layering") || lc.contains("extension audit") || lc.contains("overlayer") {
        return Some(("d365.skill.extension_audit", Some(serde_json::json!({
            "model": "GTFin"
        }))));
    }
    if lc.contains("guideline") || lc.contains("how should i think") || lc.contains("preflight") || lc.contains("pre-flight") {
        return Some(("d365.skill.aipnv_ai_pairing", Some(serde_json::json!({
