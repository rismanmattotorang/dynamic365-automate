...
async fn connect(read_only: bool) -> Arc<Client> {
    ...
    client.initialize(
        Implementation { name: "d365-smoke".into(), version: "0".into() },
...
async fn read_only_hides_write_tools_but_shows_reads() {
    ...
    assert!(!names.contains(&"xpp.meta.deploy"), "deploy must be hidden in read-only mode");
...
async fn entity_read_is_company_scoped() {
    let client = connect(true).await;
    let result = client.call_tool("d365.entity.read", Some(serde_json::json!({
    ...
    // `{ "values": { ... } }`.
    assert!(rows.iter().all(|r| r["values"]["dataAreaId"].as_str() == Some("USMF")));
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn service_metadata_reports_changeset_and_security() {
    let client = connect(true).await;
    let result = client.call_tool("d365.service.metadata", Some(serde_json::json!({
    ...
    assert!(body["security"].as_array().map(|a| !a.is_empty()).unwrap_or(false));
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn infolog_parse_detects_failure() {
    let client = connect(true).await;
    let result = client.call_tool("d365.infolog.parse", Some(serde_json::json!({
...
async fn commit_write_is_denied_in_read_only_mode() {
    ...
    let result = client.call_tool("d365.service.call", Some(serde_json::json!({
        "operation": "LedgerGeneralJournalEntryPost",
        "parameters": {"JournalBatchNumber": "000123", "JournalLines": []},
        "commit": true
    }))).await.expect("call");
    assert!(result.is_error, "commit write must be denied in read-only mode");
}
...
async fn resources_and_prompts_are_registered() {
    ...
    assert!(pnames.contains(&"d365.review-service-call"), "have: {pnames:?}");
