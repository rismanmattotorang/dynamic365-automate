            ...
            Box::pin(async move {
                let arg_map: HashMap<String, String> = match arguments {
                    Some(serde_json::Value::Object(m)) => m.into_iter()
                        .filter_map(|(k, v)| v.as_str().map(|s| (k, s.to_string()))).collect(),
                    _ => HashMap::new(),
                };
    ...
    PromptDescriptor {
        prompt: Prompt {
            ...
            arguments: skill_for_handler.arguments.iter().map(|a| PromptArgument {
                name: a.name.clone(), description: a.description.clone(), required: a.required,
            }).collect(),
        },
...
fn review_cross_reference() -> PromptDescriptor {
    ...
    impl PromptHandler for H {
        fn get(&self, arguments: Option<serde_json::Value>) -> Pin<Box<dyn Future<Output = mcp_core::Result<GetPromptResult>> + Send + '_>> {
            ...
            Box::pin(async move {
                ...
                Ok(GetPromptResult {
                    description: Some("Cross-reference review before changing or deploying an X++ object.".into()),
                    messages: vec![PromptMessage { role: Role::User, content: ToolContent::text(body) }],
                })
            })
        }
    ...
    PromptDescriptor {
        prompt: Prompt {
            ...
            description: Some("Walk the agent through a cross-reference analysis before changing an X++ object.".into()),
            arguments: vec![
                PromptArgument { name: "object".into(), description: Some("Object name".into()), required: true },
                PromptArgument { name: "kind".into(), description: Some("Object kind (class | interface | table | ...)".into()), required: false },
            ],
        },
        handler: Arc::new(H),
    }
...
fn review_service_call() -> PromptDescriptor {
    ...
    impl PromptHandler for H {
        fn get(&self, arguments: Option<serde_json::Value>) -> Pin<Box<dyn Future<Output = mcp_core::Result<GetPromptResult>> + Send + '_>> {
            let args = arguments.unwrap_or(serde_json::Value::Object(Default::default()));
            let operation = args.get("operation").and_then(|v| v.as_str()).unwrap_or("<UNKNOWN>").to_string();
            ...
            Box::pin(async move {
                ...
                Ok(GetPromptResult {
                    description: Some("Pre-execution review of a proposed d365.service.call".into()),
                    messages: vec![PromptMessage { role: Role::User, content: ToolContent::text(body) }],
                })
            })
        }
    ...
    PromptDescriptor {
        prompt: Prompt {
            ...
            description: Some("Pre-flight review of a proposed d365.service.call invocation.".into()),
            arguments: vec![
                PromptArgument { name: "operation".into(), description: Some("OData operation name".into()), required: true },
...
fn deploy_impact_analysis() -> PromptDescriptor {
    ...
    impl PromptHandler for H {
        fn get(&self, arguments: Option<serde_json::Value>) -> Pin<Box<dyn Future<Output = mcp_core::Result<GetPromptResult>> + Send + '_>> {
            let args = arguments.unwrap_or(serde_json::Value::Object(Default::default()));
            let package = args.get("package").and_then(|v| v.as_str()).unwrap_or("<PACKAGE>").to_string();
            ...
            Box::pin(async move {
                ...
                Ok(GetPromptResult {
                    description: Some("Cross-domain impact analysis for a Dynamics 365 deployable package".into()),
                    messages: vec![PromptMessage { role: Role::User, content: ToolContent::text(body) }],
                })
            })
        }
    ...
    PromptDescriptor {
        prompt: Prompt {
            ...
            description: Some("Multi-tool cross-domain impact analysis for a Dynamics 365 package deployment.".into()),
            arguments: vec![
                PromptArgument { name: "package".into(), description: Some("Deployable package name".into()), required: true },
                PromptArgument { name: "scope".into(), description: Some("Target environment (PRODUCTION / UAT / DEV)".into()), required: false },
            ],
        },
        handler: Arc::new(H),
    }
