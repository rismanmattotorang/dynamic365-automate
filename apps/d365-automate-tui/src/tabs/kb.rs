...
//! the Service metadata cache row (TTL pattern, polled from
//! `d365-cache://stats` on the live admin feed).
...
fn draw_collections(f: &mut Frame, app: &App, area: Rect) {
    ...
    if rows.is_empty() {
        rows.push(Row::new(vec![
            Cell::from("(no KB stats yet)").style(Style::default().fg(Color::DarkGray))
        ]));
    }
    ...
    .header(
        Row::new(vec!["collection", "points", "stale", "status"]).style(
            Style::default()
                .fg(Color::Cyan)
...
fn draw_cache(f: &mut Frame, app: &App, area: Rect) {
    ...
    let text = vec![
        ...
        ratatui::text::Line::from(ratatui::text::Span::styled(
            "  metadata-cache pattern · resource: d365-cache://stats · tool: d365.env.cache_stats",
            Style::default().fg(Color::DarkGray),
        )),
    ];
    let p = Paragraph::new(text).block(Block::default().borders(Borders::ALL).title(" Cache "));
