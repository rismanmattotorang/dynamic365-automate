...
use d365_automate_ingest::{EmbeddingClient, LearnCrawler, IngestionPipeline, MockEmbedder};
use d365_automate_kb::{InMemoryKb, KnowledgeStore};
use d365_automate_rag::{GraphEngine, MockReranker, Query, RagEngine};
use std::sync::Arc;
use std::time::Instant;

#[derive(Parser)]
#[command(name = "d365-automate-bench", about = "Phase 3 acceptance harness: P95 retrieval < 80 ms")]
struct Cli {
    /// Directory of *.html files to ingest.
    #[arg(long, default_value = "./docs/sample-learn-corpus")]
    input_dir: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut chunker_cfg = d365_automate_ingest::ChunkerConfig::default();
    ...
    let docs = LearnCrawler::new().crawl_directory(&cli.input_dir).await?;
    ...
    let mut layer_samples: Vec<d365_automate_rag::LatencyBreakdown> = Vec::with_capacity(cli.n);

    println!("\n→ Running {} queries ...", cli.n);
    let bench_start = Instant::now();
    for i in 0..cli.n {
        let query_text = &workload[i % workload.len()];
        let qv = embedder.embed(std::slice::from_ref(query_text)).await?.into_iter().next();
        let t0 = Instant::now();
    ...
    let layer_mean = |f: fn(&d365_automate_rag::LatencyBreakdown) -> u64| -> u64 {
...
fn graph_workload_queries() -> Vec<String> {
    vec![
        "impact of changing LedgerGeneralJournalEntry".into(),
        "where is GeneralJournalAccountEntry used".into(),
        "callers of GTFinPostJournal".into(),
        "downstream from GTFinPostable".into(),
        "trace from period_close to FIN-CORE solution".into(),
        "what depends on entity FiscalCalendarPeriod".into(),
        "objects that touch product receipt".into(),
        "what does the O2C process call".into(),
        "from GTScmGrnCheck to released product".into(),
        "Dynamics 365 Finance dependencies".into(),
    ]
}
...
fn workload_queries() -> Vec<String> {
    vec![
        ...
        "open and close ledger periods FiscalCalendarPeriod".into(),
        "product receipt arrival posting".into(),
        "purchase order receipt reversal".into(),
        "sales order invoicing free text invoice".into(),
        "trade agreement price discount journal".into(),
        "released product item model group".into(),
        "inventory valuation FIFO weighted average".into(),
        "LedgerGeneralJournalEntry general journal".into(),
        "GeneralJournalAccountEntry LedgerJournalTrans reconciliation".into(),
        "released product master ReleasedProductsV2".into(),
        "Solution Checker best practice scan".into(),
        "difference between general journal and invoice journal".into(),
        "product receipt return delivery".into(),
        "general ledger main account determination posting".into(),
        "legal entity setup CompaniesV2 currency".into(),
    ]
}
