  ...
  useEffect(() => {
    initialize().then(() => listPrompts()).then(p => {
      ...
      if (p.prompts.length > 0) {
        const first = p.prompts.find(x => x.name.startsWith('d365.skill.')) ?? p.prompts[0];
              ...
              <li key={p.name}>
                ...
                >
                  <div className="flex items-center gap-2">
                    {p.name.startsWith('d365.skill.') && <Badge tone="accent">skill</Badge>}
                    {!p.name.startsWith('d365.skill.') && <Badge tone="neutral">built-in</Badge>}
                  </div>
            ...
            <>
              ...
              <Card title="Why this matters">
                <div className="text-xs text-zinc-400 leading-relaxed space-y-2">
                  <p>
                    The guiding principle: <b>agents invoke skills, not raw tools</b>. A skill is a
                    declarative workflow template that the server exposes as an MCP prompt.
                  </p>
                  <p>
                    A skill bundles tool composition + prompt engineering for one Dynamics 365 workflow.
                  ...
                  <p>
                    ...
                    {' '}<code>d365.skill.aipnv_ai_pairing</code> captures a five-question
                    anti-autopilot pre-flight checklist for write-side calls. The gateway routes user
