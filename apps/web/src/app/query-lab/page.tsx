...
const DOMAINS = ['all', 'learn', 'xpp', 'flow', 'solution'] as const;
...
const SUGGESTED: string[] = [
  'period close GeneralJournalAccountEntry reconciliation',
  'LedgerGeneralJournalEntry journal',
  'product receipt arrival posting',
  'where is the OData action for sales order creation',
...
export default function QueryLab() {
  ...
  async function runQuery() {
    ...
    try {
      const result = await callTool('d365.docs.search', { query, top_k: topK, domain });
      ...
      <div className="p-6 space-y-6 max-w-[1500px]">
        ...
        <Card title="What you're actually seeing">
          <div className="text-xs text-zinc-400 leading-relaxed space-y-2">
            <p>
              The server's <code className="text-accent-500">d365.docs.search</code> tool runs the full Phase 3 pipeline:
            </p>
            <ol className="list-decimal pl-5 space-y-1">
              ...
              <li><b>Sparse retrieval</b> — proper BM25 (k1=1.5, b=0.75) with identifier-preserving tokeniser.</li>
              <li><b>RRF fusion</b> — Reciprocal Rank Fusion with k=60. Hits in both rankings get rewarded.</li>
              <li><b>Cross-encoder rerank</b> — MockReranker boosts exact-identifier matches (LedgerJournalTrans, ReleasedProductsV2, etc.). ONNX cross-encoder slot for Phase 7.</li>
            </ol>
...
/** Maps Dynamics 365 URIs to their colour + label. */
