...
export default function ToolExplorer() {
  ...
  useEffect(() => {
    initialize().then(() => listTools()).then(t => {
      setTools(t.tools);
      if (t.tools.length > 0) setSelected(t.tools.find(x => x.name === 'd365.docs.search')?.name ?? t.tools[0].name);
    });
  }, []);
...
function ToolForm({ tool }: { tool: Tool }) {
  ...
  // (xpp.meta.deploy; d365.service.call with require_read_only_safe=false).
  const isWriteTool = tool.name === 'xpp.meta.deploy' || tool.name === 'd365.service.call';
  ...
  return (
    <div className="space-y-2">
      {content.map((c, i) => {
        ...
        // Try to JSON-pretty-print the text payload.  All RAG/OData/Metadata tools
