...
export class McpError extends Error {
  ...
  /** Maps to the named codes from mcp_core::error::ErrorCode + odata/meta ranges. */
...
export async function initialize(): Promise<InitializeResult> {
  return rpc<InitializeResult>('initialize', {
    ...
    capabilities: {},
    clientInfo: { name: 'd365-automate-web', version: '0.1.0' },
  });
}
...
/** Helper: parse the JSON body that D365-Automate's RAG tools emit. */
